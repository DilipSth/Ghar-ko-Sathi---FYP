...
const GharUsers = () => {
  ...
  const [gharUsers, setGharUsers] = useState([]);
  ...
  useEffect(() => {
    const fetchGharUsers = async () => {
      setGharUserLoading(true);
      try {
        const response = await axios.get(
          "http://localhost:8000/api/gharUsers",
          {
            headers: {
        ...
        if (response.data.success) {
          const data = response.data.gharUsers.map((ghar) => ({
            _id: ghar._id,
            name: ghar.userId.name,
            mobile: ghar.userId.mobile || "N/A",
            email: ghar.userId.email || "N/A",
            type: ghar.userId.role || "User",
            status: ghar.status || "Active",
            profileImage: ghar.userId.profileImage,
          }));
          setGharUsers(data);
        }
      } catch (error) {
  ...
  const filteredUsers = gharUsers.filter((user) => {
              ...
              {filteredUsers.map((user, index) => (
                <tr key={index} className="border-t text-center">
                  <td className="p-4 flex justify-center items-center">
                    {user.profileImage ? (
                      <img
                        src={`http://localhost:8000/public/servicesPhotoUpload/${user.profileImage}`}
                        alt="Profile"
